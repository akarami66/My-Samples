...
AS (SELECT SCHEMA_DATA.name AS schema_name,
    ...
    WHERE TABLE_DATA.is_ms_shipped = 0
          ...
          AND INDEX_DATA.filter_definition IS NULL)
SELECT rn = ROW_NUMBER() OVER (PARTITION BY DUPE1.table_name,
                                            DUPE1.key_column_list
                               ORDER BY DUPE1.table_name,
                                        DUPE1.key_column_list,
                                        DUPE1.is_primary_key DESC
                              ),
       DUPE1.table_name,
       DUPE1.key_column_list,
       DUPE1.is_primary_key,
       DUPE1.index_name,
       DUPE1.filter_definition,
       DUPE1.is_unique_constraint
INTO #tmp
...
FROM #tmp AS t
WHERE t.rn > 1
      AND t.is_primary_key = 0
ORDER BY t.is_primary_key DESC;
SELECT *
FROM #DeleteList AS dl;
SET @cnt =
(
    SELECT COUNT(1)FROM #DeleteList
);
WHILE @cnt > 0
BEGIN
    SET @IDXName =
    (
        SELECT TOP (1) index_name FROM #DeleteList
    );
    SET @TBLName =
    (
        SELECT TOP (1) table_name FROM #DeleteList WHERE index_name = @IDXName
    );
    SET @IsUniqueConstraint =
    (
        SELECT dl.is_unique_constraint
        FROM #DeleteList AS dl
        WHERE dl.index_name = @IDXName
    );
    IF @IsUniqueConstraint = 0
    BEGIN
        SET @SqlStatement = N'DROP INDEX ' + @IDXName + N' ON ' + @TBLName + N';';
        SELECT @SqlStatement;
    END;
    ELSE
    BEGIN
        SET @SqlStatement = N'alter table ' + @TBLName + N' drop CONSTRAINT ' + @IDXName + N';';
        SELECT @SqlStatement;
    END;
    --BEGIN TRY
    --    SET @IDXName =
